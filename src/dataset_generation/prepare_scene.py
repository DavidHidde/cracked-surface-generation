...
def align_camera(
    ...
    crack_center_factor = np.average((crack_height_map > 0).nonzero(), axis=0) / np.array(crack_height_map.shape)
    ...
    for face in wall.data.polygons:
        if face.normal.y > max_y:
            max_y = face.normal.y
            chosen_face = face
    vertices = np.array(chosen_face.vertices)
    min_vertex = vertices[np.argmin(vertices[:, 0]), :]
    max_vertex = vertices[np.argmax(vertices[:, 0]), :]

    crack_center = mathutils.Vector(*min_vertex) + mathutils.Vector(*(max_vertex - min_vertex)) * mathutils.Vector(
        *crack_center_factor
    )
    camera.location = wall.matrix_world @ crack_center
    camera.rotation_euler = chosen_face.normal.to_track_quat('-Y', 'Z').to_euler() + [
        [np.pi / 2 + rotation[0], rotation[1], rotation[2]]]
