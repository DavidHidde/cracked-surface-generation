...
def generate_pivot_point(
    ...
    ], dtype=np.int32) * brick_projected_size
...
def determine_start_point(
    ...
    all_points.sort(key=lambda point: surface.height_map[point[1], point[0]])

    # Choose one of the 25% lowest points
    center = all_points[np.random.randint(int(len(all_points) * 0.25))]
    angle = (0 if pivot_direction == PIVOT_DIRECTION_RIGHT else np.pi) if center[1] > 0 else np.pi / 2.

    return Point(angle, initial_width, center)
